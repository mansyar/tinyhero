...
func __get_session_header(token : String = _auth) -> PackedStringArray :
	return PackedStringArray([_bearer[0] % ( token if not token.is_empty() else _config.supabaseKey )])
...
func set_auth(token : String) -> void:
	_auth = token
